# WASM build of SamCoupeWeb using Emscripten
...
"@
    ...
    $deps = @{
        ...
        "zlib"     = "https://github.com/madler/zlib/archive/51b7f2abdade71cd9bb0e7a373ef2610ec6f9daf.zip"
    }
    ...
    # 7. Path mismatch check to prevent stale cache errors
    if (Test-Path "$build_dir/CMakeCache.txt") {
        $cache_content = Get-Content "$build_dir/CMakeCache.txt" -Raw
        if ($cache_content -match 'CMAKE_CACHEFILE_DIR:INTERNAL=(.*)') {
            $last_dir = $matches[1].Trim().Replace('\', '/')
            $curr_dir = (Get-Item $build_dir).FullName.Replace('\', '/')
            if ($last_dir -ne $curr_dir) {
                Write-Host "Build directory path mismatch. Cleaning stale cache..." -ForegroundColor Yellow
                Remove-Item "$build_dir/CMakeCache.txt" -Force
                if (Test-Path "$build_dir/CMakeFiles") { Remove-Item "$build_dir/CMakeFiles" -Recurse -Force }
            }
        }
    }
    ...
    Pop-Location
    Write-Host "SamCoupeWeb WASM build completed successfully!"
    ...
    $artifacts = @("samcoupeweb.wasm", "samcoupeweb.js", "samcoupeweb.data", "samcoupeweb.wasm.map")
    foreach ($art in $artifacts) {
        $src = Join-Path $build_dir $art
        if (Test-Path $src) {
            Write-Host "Copying $art..." -ForegroundColor Gray
            Copy-Item $src $deploy_dir -Force
        } elseif ($art -ne "samcoupeweb.wasm.map") {
